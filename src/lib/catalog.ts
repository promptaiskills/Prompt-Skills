...
function matchCategory(value: string): TaxonomyCategory | null {
  ...
  if (byNorm && byNorm.length > 1) {
    // identical titles in different collections — pick the first (deterministic)
    return byNorm[0];
  }
  // prefix match (≥5 chars)
  let best: TaxonomyCategory | null = null;
  for (const [nrm, cats] of catByNorm) {
...
function matchSubcategory(category: TaxonomyCategory | null, value: string) {
  const v = normalizeForMatch(value);
  if (!v) return null;
  const exact = subByKey.get(v);
  if (exact) return exact;
  const pool = category ? subByNormGlobal.get(v)?.filter((s) => s.category.key === category.key) : subByNormGlobal.get(v);
  if (pool && pool.length === 1) return subByKey.get(pool[0].key);
  if (pool && pool.length > 1) return subByKey.get(pool[0].key);
  // prefix
  let best: (TaxonomySub & { category: TaxonomyCategory; collection: TaxonomyCollection }) | null = null;
  for (const [nrm, subs] of subByNormGlobal) {
    if (v.length >= 5 && (nrm.startsWith(v) || v.startsWith(nrm))) {
      const candidates = category ? subs.filter((s) => s.category.key === category.key) : subs;
      if (candidates.length) {
        const picked = candidates[0];
        const full = subByKey.get(picked.key);
        if (full) {
          best = full;
...
function buildCatalog() {
  ...
  for (const r of rows) {
    ...
    const sub = matchSubcategory(category, r.subcategory);
    if (!category) warnings.push(`${id}: category "${r.category}" did not match the taxonomy (fix the CSV or taxonomy).`);
    if (!sub) warnings.push(`${id}: subcategory "${r.subcategory}" did not match the taxonomy (fix the CSV or taxonomy).`);
